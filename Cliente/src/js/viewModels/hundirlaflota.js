...
define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'ojs/ojknockout', 'ojs/ojdatagrid', 'ojs/ojarraydatagriddatasource'],
 function(oj, ko, $, app) {
    ...
    function HFViewModel() {
		...
		self.misbarcosrestantes = ko.observable("3");
		self.barcosrestantesenemigo = ko.observable("3");
		self.mitablero=ko.observable();
			
		self.tableroenemigo=ko.observable(new oj.ArrayDataGridDataSource([
				[' ',' ',' '],
				[' ',' ',' '],
				[' ',' ',' ']
			]));
		...
		self.barcosrestantesenemigobind = ko.computed(function() {
			return JSON.stringify(self.barcosrestantesenemigo());
			
			
		});self.misbarcosrestantesbind = ko.computed(function() {
			return JSON.stringify(self.misbarcosrestantes());
		...
		function hacermovimiento(mensaje){
			if (self.jugador() != self.turno()){
				self.mitablero(new oj.ArrayDataGridDataSource(mensaje.tableroatacado));
				
			} else {
				self.tableroenemigo(new oj.ArrayDataGridDataSource(mensaje.tableroatacante));
			}
			self.turno(mensaje.turno);
			
			if (self.jugador() == self.jugador1()){
				self.misbarcosrestantes(mensaje.barcosrestantes1);
				self.barcosrestantesenemigo(mensaje.barcosrestantes2);
			} else if (self.jugador() == self.jugador2()){
				self.misbarcosrestantes(mensaje.barcosrestantes2);
				self.barcosrestantesenemigo(mensaje.barcosrestantes1);
			}
		...
		function mostrarinforpartida(mensaje){
			self.jugador1(mensaje.jugador1);
			self.jugador2(mensaje.jugador2);
			self.turno(self.jugador1());
			
			//por seguridad
			if (self.jugador() == self.jugador1()){
				self.mitablero(new oj.ArrayDataGridDataSource(mensaje.tableroJugador1));
			} else if (self.jugador() == self.jugador2()){
				self.mitablero(new oj.ArrayDataGridDataSource(mensaje.tableroJugador2));
			}
		}
